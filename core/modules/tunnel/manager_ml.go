...
func (m *Manager) mlBlockmapSync(ctx context.Context) {
	...
	if outcomes := m.transportAgent.DrainOutcomes(); len(outcomes) > 0 {
		body, _ := json.Marshal(neural.BlockReport{CC: cc, ASN: asn, Reports: outcomes})
...
func (m *Manager) runWeightSnapshotLoop() {
	...
	defer ticker.Stop()
	neural.SetGlobalSnapshot(m.ExportMLWeights())
	for range ticker.C {
		neural.SetGlobalSnapshot(m.ExportMLWeights())
	}
}

func (m *Manager) ExportMLWeights() *neural.WeightSnapshot {
...
func (m *Manager) ImportMLWeights(snap *neural.WeightSnapshot) {
	if snap == nil {
		return
	}
