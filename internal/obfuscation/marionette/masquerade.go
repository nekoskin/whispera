...
// --- Behavioral Mimicry (formerly behavioral_mimicry.go) ---
...
// applyJA3Evasion returns the JA3 fingerprint (CipherSuites) from the active uTLS connection
// Duplicate removed - see line 558
...
// --- Evasion ML (formerly marionette_evasion_ml.go) ---
...
// --- Evasion Pool (formerly marionette_evasion_pool.go) ---
...
// --- Evasion Protocols (formerly marionette_evasion_protocols.go) ---

// Real browser JA3 fingerprints (from uTLS)
var realJA3Fingerprints = map[string][]byte{
	"chrome": {
		// Chrome 120 on Windows JA3 hash components (TLS version, cipher suites, extensions)
		0x03, 0x03, // TLS 1.2
		0x13, 0x01, 0x13, 0x02, 0x13, 0x03, // TLS 1.3 cipher suites
		0xc0, 0x2b, 0xc0, 0x2f, 0xc0, 0x2c, 0xc0, 0x30, // ECDHE suites
		0x00, 0x00, 0x00, 0x17, 0x00, 0x2b, 0x00, 0x0d, // Extensions
	},
	"firefox": {
		// Firefox 121 JA3 components
		0x03, 0x03,
...
func (m *Marionette) applyJA3Evasion(data []byte) []byte {
	// First check if we have a real uTLS connection to extract fingerprint from
	m.Mutex.RLock()
	conn := m.UTLSConn
	m.Mutex.RUnlock()
	...
	// Get fingerprint based on UTLSFingerprint setting
	fp := m.UTLSFingerprint
	if fp == "" {
		fp = "chrome" // Default to Chrome
	}

	// Get real fingerprint for this browser
	...
	// Fallback: randomize between browsers
...
func (m *Marionette) applyJA4Evasion(data []byte) []byte {
	...
	// Get real JA4 fingerprint for this browser
...
// --- Website Defense (formerly marionette_defense_website.go) ---
// Note: Keeping minimal logic here as defined in the source file.
...
// --- Advanced Mimicry (formerly marionette_mimicry_advanced.go) ---
// Since we have a lot of mimicry logic, let's keep it here too.
