...
// Reference methods/functions to silence staticcheck unused warnings
var _ = []interface{}{
...
// --- Resilience (formerly marionette_resilience.go) ---
...
// --- Memory Maintenance (formerly marionette_maintenance.go) ---
...
// --- Randomness & Math Helpers (formerly marionette_random.go & marionette_helpers.go) ---
...
// --- Buffer Pools (formerly marionette_pools.go) ---
...
// processWithTimeout processes data using ML with timeout, using channel pools
func (m *Marionette) processWithTimeout(data []byte, processor func([]byte) ([]byte, error), timeout time.Duration) ([]byte, error) {
	...
	case <-time.After(timeout):
		return data, nil // Graceful degradation on timeout
	}
}

// processWithBufferReuse processes data reusing buffers from pool
func (m *Marionette) processWithBufferReuse(data []byte, transform func([]byte)) []byte {
	buf := getBufferFromPool(len(data))
	buf = append(buf, data...)
	
	// Apply transformation in-place on the buffer
	transform(buf)
