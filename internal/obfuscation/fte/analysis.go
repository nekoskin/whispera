...
// Traffic analysis and profile management logic
...
func (fte *FTE) updateProfilesFromRealData(analysis *TrafficAnalysis) {
	...
	// Update profiles based on analysis
}

// Entropy calculation and adjustment logic
...
func (fte *FTE) getMLBasedSizeWeights(profile *ProtocolProfile) []float64 {
	...
	// Use Direction and Timestamp to adjust weights
	weights := make([]float64, len(profile.CommonSizes))
	for i := range weights {
		weights[i] = 1.0
	}
	...
	if context.Direction == "inbound" {
		// Inbound traffic typically has different size distribution
		for i := range weights {
			if profile.CommonSizes[i] > 512 {
				weights[i] *= 1.2 // Larger packets more likely for responses
			}
		}
	}
	...
	case "vk":
		// VK prefers medium-sized packets for messaging
		for i := range weights {
	...
	case "yandex":
		// Yandex search has bimodal distribution
		for i := range weights {
	...
	// Adjust based on Size (number of common sizes available)
	if context.Size > 5 {
		// More size options means more granular control
		for i := range weights {
			weights[i] *= 1.0 + 0.05*float64(context.Size-5)
		}
	}

	// Time-based adjustment (business hours vs off-hours)
	hour := context.Timestamp.Hour()
	if hour >= 9 && hour <= 18 {
		// Business hours: more varied traffic
		for i := range weights {
