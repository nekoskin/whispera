...
type ProtocolRecommendation struct {
	Protocol              string  // "tls", "dtls", "noise_ik", "websocket", "http2", "grpc"
...
func (ps *ProtocolSelector) analyzeMLPrediction(response *types.MLPredictionResponse, conditions *NetworkConditions) *ProtocolRecommendation {
	...
	switch {
	case pred.DPIType >= 5: // Advanced Heuristics - используем gRPC для маскировки под API
	...
	case pred.DPIType >= 4: // ML-based Detection - используем HTTP/2 или TLS
		if conditions.Bandwidth > 50 { // Высокая пропускная способность - HTTP/2 мультиплексирование
