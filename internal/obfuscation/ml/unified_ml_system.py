...
class UnifiedMLSystem:
    ...
    def _prepare_features(self, packet_data: bytes) -> np.ndarray:
        ...
        features[0] = len(packet_data) / 1500.0
        features[1] = np.mean(packet_array) / 255.0
        features[2] = np.std(packet_array) / 255.0
        features[3] = np.var(packet_array) / 255.0
        features[4] = np.min(packet_array) / 255.0
        features[5] = np.max(packet_array) / 255.0
        features[6] = np.median(packet_array) / 255.0
        features[7] = np.percentile(packet_array, 25) / 255.0
        features[8] = np.percentile(packet_array, 75) / 255.0
        features[9] = np.percentile(packet_array, 90) / 255.0
        
        features[10] = np.percentile(packet_array, 95) / 255.0
        features[11] = np.percentile(packet_array, 99) / 255.0
        features[12] = np.sum(packet_array == 0) / len(packet_array)
        features[13] = np.sum(packet_array == 255) / len(packet_array)
        features[14] = np.sum(packet_array > 128) / len(packet_array)
        
        if len(packet_data) >= 20:
            features[15] = packet_data[0] / 255.0
            features[16] = packet_data[1] / 255.0
            features[17] = (packet_data[2] << 8 | packet_data[3]) / 65535.0
            features[18] = packet_data[8] / 255.0
            features[19] = packet_data[9] / 255.0
            
            if len(packet_data) >= 40 and packet_data[9] == 6:
                features[20] = (packet_data[20] << 8 | packet_data[21]) / 65535.0
                features[21] = (packet_data[22] << 8 | packet_data[23]) / 65535.0
                features[22] = packet_data[33] / 255.0
                features[23] = (packet_data[34] << 8 | packet_data[35]) / 65535.0
        ...
        byte_counts = byte_counts / (len(packet_array) + 1e-10)
