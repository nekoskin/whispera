...
func (s *SOCKS5Server) ListenAndServe() error {
	...
	log.Printf("[SOCKS5] ✅ Server listening on %s - ready to accept connections", s.listenAddr)
	
	for {
		conn, err := listener.Accept()
		if err != nil {
			log.Printf("[SOCKS5] Failed to accept connection: %v", err)
			continue
		}
...
func (s *SOCKS5Server) handleConnection(conn net.Conn) {
	...
	if err := s.handler(conn, addr, port); err != nil {
		log.Printf("[SOCKS5] Handler failed: %v", err)
	}
}
