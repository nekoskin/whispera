...
func TestStreamFlowControl(t *testing.T) {
	...
	// Try to consume more than window
...
func TestHalfCloseLifecycle(t *testing.T) {
	...
	// Cleanup should remove half-closed stream after timeout
	removed := mux.CleanupInactive(-1) // timeout negative to trigger immediate removal
	if removed != 1 {
