...
class WhisperaAPI {
    ...
    async addInbound(inbound) {
        return this.request('/api/inbounds/add', {
            method: 'POST',
    ...
    async deleteInbound(tag) {
        return this.request('/api/inbounds/delete', {
            method: 'POST',
    ...
    async updateAllSubscriptions() {
        ...
        for (const s of list) {
            if (s && s.id != null) {
                try { await this.updateSubscription(s.id, s); updated++; } catch (_) {}
            }
        }
    ...
    async getBridges() {
        return this.request('/api/bridge-list');
    }
...
export const api = new WhisperaAPI();
window.api = api;
