...
class ProductionDataCollector:
    ...
    def __init__(self, output_dir: str = "production_data", min_samples: int = 10000):
        ...
        self.collected_data = {
            ...
            'anomaly_labels': [],
            'timestamps': [],
            'packet_hashes': set()  # Для дедупликации
        }
    ...
    def start_collection(self, duration_minutes: int = 60, interface: str = None) -> Dict:
        ...
        def packet_handler(packet):
            ...
            # ИСПРАВЛЕНО: Сначала увеличиваем общий счетчик
            ...
            try:
                ...
                if features is None:
                    # ИСПРАВЛЕНО: Это больше не должно происходить, но на всякий случай
                ...
                # ИСПРАВЛЕНО: Увеличиваем только валидные пакеты
    ...
    def _extract_features_safe(self, packet) -> Optional[np.ndarray]:
        ...
        try:
            ...
            # Базовые характеристики пакета
            packet_size = len(packet)
            if packet_size == 0:
                # ИСПРАВЛЕНО: Не отклоняем пустые пакеты, заполняем базовыми значениями
                features[idx] = 0.0  # Размер = 0
                idx += 1
                features[idx] = 0.0  # Время = 0
                idx += 1
                # Остальные признаки остаются нулевыми
                return features
            ...
            # Дополняем оставшиеся признаки
            while idx < TARGET_FEATURES:
                features[idx] = 0.0
                idx += 1
            
            # ИСПРАВЛЕНО: Более мягкая валидация признаков
            if np.any(np.isnan(features)):
                # Заменяем NaN на 0
                features = np.nan_to_num(features, nan=0.0)
            
            if np.any(np.isinf(features)):
                # Заменяем Inf на разумные значения
                features = np.nan_to_num(features, posinf=1.0, neginf=0.0)
            
            return features
            
        except Exception as e:
            # ИСПРАВЛЕНО: Не возвращаем None, возвращаем базовые признаки
            self.logger.warning(f"Ошибка извлечения признаков: {e}, используем базовые признаки")
            features = np.zeros(100, dtype=np.float32)
            features[0] = 0.1  # Базовый размер пакета
            features[1] = 0.5  # Базовое время
            return features
    ...
    def _calculate_quality_score(self):
        ...
        # ИСПРАВЛЕНО: Проверяем логику подсчета
        valid_packets = self.quality_metrics['valid_packets']
        duplicate_packets = self.quality_metrics['duplicate_packets']
        invalid_packets = self.quality_metrics['invalid_packets']
        
        # ИСПРАВЛЕНО: Валидируем данные
        ...
        # ИСПРАВЛЕНО: Более мягкая формула качества
    ...
    def _save_collected_data(self):
        ...
        # Сохраняем validated данные (только качественные)
        if self.quality_metrics['quality_score'] > 0.9:  # Повышенный порог качества
...
def collect_production_data(duration_minutes: int = 90, min_samples: int = 15000) -> Dict:
    ...
    # УЛУЧШЕННАЯ ВАЛИДАЦИЯ: Более строгие критерии
    quality_threshold = 0.85  # УВЕЛИЧЕНО: Было 0.9, стало 0.85 для более реалистичных требований
...
if __name__ == "__main__":
    # УЛУЧШЕННЫЙ запуск сбора данных
    import sys
    
    duration = 90  # УВЕЛИЧЕНО: Было 60, стало 90 минут
    min_samples = 15000  # УВЕЛИЧЕНО: Было 10000, стало 15000 образцов
