...
class AnomalyDetector:
    ...
    def build_autoencoder(self, input_size: int = 100) -> tf.keras.Model:
        ...
        # ИСПРАВЛЕНО: Сбалансированная архитектура с разумной регуляризацией
        ...
        # Decoder с симметричной архитектурой
    ...
    def train(self, X_train: np.ndarray, epochs: int = 200, batch_size: int = 32) -> Dict:
        ...
        if self.method == "autoencoder":
            # УЛУЧШЕННЫЕ callbacks для Autoencoder
            callbacks = [
            ...
            # ИСПРАВЛЕНО: Более разумный расчет порога
            train_pred = self.model.predict(X_scaled, verbose=0)
            train_errors = np.mean(np.square(X_scaled - train_pred), axis=1)
            
            # Адаптивный порог на основе статистики ошибок
            mean_error = np.mean(train_errors)
            std_error = np.std(train_errors)
            # ИСПРАВЛЕНО: Более консервативный порог для production
            self.threshold = mean_error + 1.5 * std_error  # Разумный порог
    ...
    def batch_detect(self, packets: np.ndarray) -> List[Tuple[bool, float]]:
        ...
        if self.method == "autoencoder":
            # Reconstruction error для всех пакетов
    ...
    def load_model(self, path: str):
        ...
        # Загружаем scaler (предупреждения подавлены глобально)
        import joblib
        self.scaler = joblib.load(os.path.join(path, 'scaler.pkl'))
        
        # Загружаем модель
        if self.method == "autoencoder":
            # ИСПРАВЛЕНО: Загружаем модель с custom_objects для обработки проблем с сериализацией 'mse'
            # Проблема: модель была сохранена с loss='mse' (строка), и Keras пытается найти её в keras.metrics
            # вместо keras.losses. Предоставляем явные маппинги для обоих случаев.
            model_file = os.path.join(path, 'anomaly_detector.h5')
            
            # ИСПРАВЛЕНО: Проблема в том, что модель была сохранена с 'mse' в keras.metrics,
            # но 'mse' это loss function, не metric. Нужно зарегистрировать 'mse' так,
            # чтобы Keras мог найти его в metrics namespace при десериализации.
            custom_objects = {}
            
            # Создаем loss function
            mse_loss = tf.keras.losses.MeanSquaredError()
            
            # КРИТИЧЕСКИ ВАЖНО: Модель ищет 'mse' в keras.metrics модуле.
            # Регистрируем его там через monkey-patching ПЕРЕД загрузкой модели
            try:
                import keras.metrics
                # Добавляем 'mse' в metrics namespace
                if not hasattr(keras.metrics, 'mse'):
                    keras.metrics.mse = mse_loss
            except Exception as e:
                pass
            
            # Также регистрируем в custom_objects
            custom_objects['mse'] = mse_loss
            
            # Регистрируем через keras.saving для глобальной доступности
            try:
                import keras.saving
            ...
            try:
                # Пытаемся загрузить с custom_objects и compile=False
                ...
                # Если все еще ошибка, пробуем загрузить только веса
                if "could not locate" in error_msg or "mse" in error_msg:
                    try:
                        # Последняя попытка: загружаем архитектуру и веса отдельно
                        # Но для этого нужно знать архитектуру, поэтому просто пробуем без custom_objects
                        # но с safe_mode отключенным
                        ...
                        # Если ничего не помогло, пробрасываем ошибку
                        raise ValueError(
            ...
            try:
                ...
                # Если компиляция не удалась, это не критично для inference
            ...
            # Загружаем sklearn модель (предупреждения подавлены глобально)
            import joblib
